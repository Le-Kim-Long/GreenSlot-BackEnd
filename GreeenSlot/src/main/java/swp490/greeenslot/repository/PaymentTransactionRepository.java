...
public interface PaymentTransactionRepository extends JpaRepository<PaymentTransaction, Long> {
    ...
    @Query("SELECT t FROM PaymentTransaction t WHERE t.status = 'SUCCESS' AND t.paymentDate BETWEEN :start AND :end ORDER BY t.paymentDate DESC")
    List<PaymentTransaction> findSuccessfulTransactionsBetween(
            @Param("start") LocalDateTime start,
            @Param("end") LocalDateTime end
    );

    @Query("SELECT t FROM PaymentTransaction t WHERE t.status = 'SUCCESS' AND t.paymentDate BETWEEN :start AND :end AND t.rental.gardenSlot.location.id = :locationId ORDER BY t.paymentDate DESC")
