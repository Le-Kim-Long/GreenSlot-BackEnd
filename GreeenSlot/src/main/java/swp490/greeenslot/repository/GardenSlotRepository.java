...
public interface GardenSlotRepository extends JpaRepository<GardenSlot, Long> {
    ...
    List<GardenSlot> findByLocationIdAndStatus(Long locationId, ESlotStatus status);

    List<GardenSlot> findByLocationId(Long locationId);

    @Query("SELECT g FROM GardenSlot g WHERE g.status = :status AND (:locationId IS NULL OR g.location.id = :locationId)")
    ...
    @Query("SELECT g FROM GardenSlot g WHERE g.status = :status AND g.location.id = :locationId AND g.price >= :minPrice AND g.price <= :maxPrice")
